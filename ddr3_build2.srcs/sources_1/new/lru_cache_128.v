...
module lru_cache_128 (
  ...
  localparam WORD_SIZE = 8'h80;
  localparam AGE_BITS = 2'h3;
  
  
  localparam WORDS_PER_LINE = 8'h01;
  
  localparam BYTES_PER_WORD = 8'h10;
  
  localparam SUB_ADDR_BITS = 1'h0;
  
  localparam ADDR_SIZE = 6'h18;
  ...
  reg [0:0] M_valid_d, M_valid_q = 1'h0;
  reg [2:0] M_age_d, M_age_q = 1'h0;
  reg [0:0] M_active_entry_d, M_active_entry_q = 1'h0;
  reg [127:0] M_read_data_d, M_read_data_q = 1'h0;
  reg M_read_valid_d, M_read_valid_q = 1'h0;
  reg M_read_pending_d, M_read_pending_q = 1'h0;
  reg [23:0] M_read_addr_d, M_read_addr_q = 1'h0;
  reg M_write_pending_d, M_write_pending_q = 1'h0;
  reg [23:0] M_write_addr_d, M_write_addr_q = 1'h0;
  reg [127:0] M_write_data_d, M_write_data_q = 1'h0;
  ...
  always @* begin
    ...
    M_read_valid_d = 1'h0;
    for (i = 1'h0; i < 8'h01; i = i + 1) begin
      mem_in[161+(i * 8'h10)*1+15-:16] = {8'h10{~M_valid_q}};
    end
    max_age = 1'h0;
    oldest_entry = 1'h0;
    handled = 1'h0;
    ...
    case (M_read_state_q)
      IDLE_read_state: begin
        if (rd_cmd_valid) begin
          ...
          handled = 1'h0;
          for (i = 1'h0; i < 1'h1; i = i + 1) begin
            if (!handled && M_active_q && M_valid_q && (M_address_q == rd_addr)) begin
              handled = 1'h1;
              M_read_valid_d = 1'h1;
              M_read_data_d = M_buffer_q;
              M_age_d = 1'h0;
            end
          end
          if (!handled) begin
      ...
      FETCH_read_state: begin
        ...
        handled = 1'h0;
        for (i = 1'h0; i < 1'h1; i = i + 1) begin
          if (!handled && M_active_q && M_valid_q && (M_address_q == M_read_addr_q)) begin
            handled = 1'h1;
            M_read_valid_d = 1'h1;
            M_read_data_d = M_buffer_q;
            M_age_d = 1'h0;
    ...
    case (M_write_state_q)
      IDLE_write_state: begin
        if (wr_valid) begin
          ...
          handled = 1'h0;
          for (i = 1'h0; i < 1'h1; i = i + 1) begin
            if (!handled && M_active_q && (M_address_q == wr_addr)) begin
              handled = 1'h1;
              M_written_d = 1'h1;
              M_valid_d = 1'h1;
              M_buffer_d = wr_data;
      ...
      PUT_write_state: begin
        ...
        handled = 1'h0;
        for (i = 1'h0; i < 1'h1; i = i + 1) begin
          if (!handled && M_active_q && (M_address_q == M_write_addr_q)) begin
            handled = 1'h1;
            M_written_d = 1'h1;
            M_valid_d = 1'h1;
            M_buffer_d = M_write_data_q;
            M_age_d = 1'h0;
          end
        end
        if (handled) begin
    ...
    case (M_state_q)
      IDLE_state: begin
        if (flush) begin
          ...
          if (M_read_pending_q) begin
            ...
            if (M_active_q && M_address_q != M_read_addr_q) begin
              M_active_d = 1'h0;
              M_state_d = PREP_READ_ENTRY_state;
            end else begin
              M_state_d = READ_CMD_state;
              M_address_d = M_read_addr_q;
              if (M_address_q != M_read_addr_q) begin
                M_valid_d = 1'h0;
              end
            end
          end else begin
      ...
      WAIT_READ_state: begin
        if (mem_out[128+0-:1]) begin
          for (i = 1'h0; i < 8'h01; i = i + 1) begin
            if (!M_valid_q) begin
              M_buffer_d = mem_out[0+(8'h80 * i)*1+127-:128];
            end
          end
          M_active_d = 1'h1;
          M_valid_d = 1'h1;
