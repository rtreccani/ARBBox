    ...
    case (M_read_state_q)
      IDLE_read_state: begin
        if (rd_cmd_valid) begin
          ...
          handled = 1'h0;
          for (i = 1'h0; i < 1'h1; i = i + 1) begin
            if (!handled && M_active_q && M_valid_q[(i)*16+(1'h1 ? rd_addr[0+3-:4] : 1'h0)*1+0-:1] && (M_address_q == rd_addr[4+23-:24])) begin
              handled = 1'h1;
              M_read_valid_d = 1'h1;
              M_read_data_d = M_buffer_q[(i)*128+(1'h1 ? rd_addr[0+3-:4] : 1'h0)*8+7-:8];
              M_age_d = 1'h0;
            end
          end
          if (!handled) begin
    ...
    case (M_write_state_q)
      IDLE_write_state: begin
        if (wr_valid) begin
          ...
          handled = 1'h0;
          for (i = 1'h0; i < 1'h1; i = i + 1) begin
            if (!handled && M_active_q && (M_address_q == wr_addr[4+23-:24])) begin
              handled = 1'h1;
              M_written_d = 1'h1;
              M_valid_d[(i)*16+(1'h1 ? wr_addr[0+3-:4] : 1'h0)*1+0-:1] = 1'h1;
              M_buffer_d[(i)*128+(1'h1 ? wr_addr[0+3-:4] : 1'h0)*8+7-:8] = wr_data;
              M_age_d = 1'h0;
            end
          end
          if (!handled) begin
